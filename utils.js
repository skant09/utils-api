    ...
    store.dispatch(
      setAlert({
        ...
        message: userMessage.concat(
          `${apiResponse.system_message && +apiResponse.status === 422 ? `. ${apiResponse.system_message}` : ''}`
        ),
      })
    ),
  300
...
const { getApi, postApi, handleApiResponse } = fetchAPI(apiErrorHandler, requestIntercepter);
